...
namespace ICSharpCode.SharpZipLib.Tar
{
	...
	public class TarArchive : IDisposable
	{
        ...
        /// </param>
        /// <see cref="FileOverwriteOption"/>
        [Obsolete("Use the FileOverwriteOption property")]
        public void SetKeepOldFiles(bool keepExistingFiles)
		{
            ...
            if (keepExistingFiles)
                this.FileOverwriteOption = FileOverWriteOptions.Never;
            else
                this.FileOverwriteOption = FileOverWriteOptions.Always;
		}

        /// <summary>
        /// File Overwrite actions
        /// </summary>        
        /// <seealso cref="FileOverwriteOption"/>
        public enum FileOverWriteOptions
        {
            /// <summary>
            /// Will always overwrite the existing file. This is the default
            /// </summary>
            Always = 0,
            /// <summary>
            /// The old file is not overwritten
            /// </summary>
            Never,
            /// <summary>
            /// The existing file is overwritten only if the new file is newer.
            /// </summary>
            OnlyIfNewer,
            /// <summary>
            /// The existing file is not overwritten but the new file will be renamed
            /// This option will allow concurrent processing.
            /// </summary>
            Rename,
            /// <summary>
            /// Keep existing file and do not write new file if the new file has the same write timestamp and size otherwise the new file is written with a different file name.
            /// </summary>
            RenameOnlyIfDifferent,
            /// <summary>
            /// An exception is thrown (System.IO.IOException) if the file already exists
            /// </summary>
            Exception
        }

        /// <summary>
        /// Defines how files are overwritten if the extracted file already exists.
        /// </summary>
        /// <seealso cref="FileOverWriteOptions"/>
        public FileOverWriteOptions FileOverwriteOption
        {
            get;
            set;
        }
		...
		/// <summary>
		/// Get/set a value indicating whether the file dates and times should
		...
		public void ExtractContents(string destinationDirectory)
		{
			...
			while (true) {
				...
				var outFileInfo = ExtractEntry(destinationDirectory, entry);

                if(RestoreDateTimeOnExtract
                        && outFileInfo != null) {
                    outFileInfo.LastWriteTimeUtc = DateTime.SpecifyKind(entry.ModTime, DateTimeKind.Utc);
                    outFileInfo.LastAccessTimeUtc = outFileInfo.LastWriteTimeUtc;
                }
			}
		}

        /// <summary>
        /// Used to ensure renaming a file is valid during concurrent operations
        /// </summary>
		...
		/// </param>
		FileInfo ExtractEntry(string destDir, TarEntry entry)
		{
			OnProgressMessageEvent(entry, null);
			...
			if (entry.IsDirectory) {
				...
				if (fileInfo.Exists) {
					if (this.FileOverwriteOption == FileOverWriteOptions.Never) {
                    ...
                    if(process && this.FileOverwriteOption != FileOverWriteOptions.Always) {
                        if (this.FileOverwriteOption == FileOverWriteOptions.Exception) {
                            throw new System.IO.IOException(string.Format("Destination file {0} already exists.", destFile));
                        }

                        if (this.FileOverwriteOption == FileOverWriteOptions.Rename) {
                            lock (SyncRoot) {
                                var fileName = System.IO.Path.GetFileNameWithoutExtension(name);
                                var fileExt = System.IO.Path.GetExtension(name);
                                var dirName = System.IO.Path.GetDirectoryName(name);

                                for (int tryNbr = 2; ; ++tryNbr) {
                                    destFile = Path.Combine(destDir, dirName, string.Format("{0} ({1}){2}", fileName, tryNbr, fileExt));
                                    fileInfo = new FileInfo(destFile);
                            ...
                            OnProgressMessageEvent(entry, "Destination file already exists but the new file will be renamed");
                        }
                        else if (this.FileOverwriteOption == FileOverWriteOptions.RenameOnlyIfDifferent)
                        {
                            lock (SyncRoot)
                            {
                                var newFile = DateTime.SpecifyKind(entry.ModTime, DateTimeKind.Utc);
                                var newSize = entry.Size;
                                var existing = fileInfo.LastWriteTimeUtc;
                                var existingSize = fileInfo.Length;

                                if (newFile == existing && newSize == existingSize)
                                {
                                    process = false;
                                }
                                else
                        ...
                        else if (this.FileOverwriteOption == FileOverWriteOptions.OnlyIfNewer) {
                            var newFile = DateTime.SpecifyKind(entry.ModTime, DateTimeKind.Utc);
                            var existing = fileInfo.LastWriteTimeUtc;
                            ...
                            if(newFile <= existing) {
                                ...
                                OnProgressMessageEvent(entry, "Destination file already exists and will be overwritten");
                            }
                        }
                    }
				}
