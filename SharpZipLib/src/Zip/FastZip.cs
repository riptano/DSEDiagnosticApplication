		...
		void ExtractFileEntry(ZipEntry entry, string targetName)
		{
			...
			if ( overwrite_ != Overwrite.Always ) {
				if ( File.Exists(targetName) ) {
                    ...
                    else if (overwrite_ == Overwrite.Exception) {
                        throw new System.IO.IOException(string.Format("Destination file {0} already exists.", targetName));
                    }
                    else if(overwrite_ == Overwrite.OnlyIfNewer) {
                        var newFile = DateTime.SpecifyKind(entry.DateTime, DateTimeKind.Utc);
                        var existing = File.GetLastWriteTimeUtc(targetName);
                    ...
                    else if(overwrite_ == Overwrite.Rename)
                    {
                        lock (SyncRoot)
                        {
                            var fileName = System.IO.Path.GetFileNameWithoutExtension(targetName);
                            var fileExt = System.IO.Path.GetExtension(targetName);
                            var destDir = System.IO.Path.GetDirectoryName(targetName);
                            FileInfo fileInfo = null;

                            for (int tryNbr = 2; ; ++tryNbr)
                            {
                                targetName = Path.Combine(destDir, string.Format("{0} ({1}){2}", fileName, tryNbr, fileExt));
                                fileInfo = new FileInfo(targetName);
                    ...
                    else if (overwrite_ == Overwrite.RenameOnlyIfDifferent)
                    {
                        lock (SyncRoot)
                        {
                            var newFile = DateTime.SpecifyKind(entry.DateTime, DateTimeKind.Utc);
                            var newSize = entry.Size;
                            var fileInfo = new FileInfo(targetName);
