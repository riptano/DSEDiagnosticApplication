...
Arguments including Default Values:
	...
	-D, --DiagnosticPath [Default Value "C:\Users\richard.andersen\Documents\DSEData"] The directory location of the diagnostic files. The structure of these folders and files is depending on the value of DiagnosticNoSubFolders. If Relative Path, this path is merged with current default directory path. The defaults are defined in the DiagnosticPath app-config file.
	...
	-E, --ExcelFileTemplatePath [Default Value "C:\\Bin\HealthCheck\DSEDiagnosticApplication\DSEDiagnosticConsoleApplication\bin\Release\dseHealthAssessment3Template.xlsm"] Excel Template file that is used to create Excel target file (ExcelFilePath), if it doesn't already exists.
	...
	--ProcessFileMappingPath [Default Value "C:\\Bin\HealthCheck\DSEDiagnosticApplication\DSEDiagnosticConsoleApplication\bin\Release\Json\ProcessFileMappingsValidate.json"] File Mapper Json file used to define which and how files are processed.
